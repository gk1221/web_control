...
if DBaction="a" or DBaction="u" then 
		...
		rs.open "select * from config where Kind='資訊中心' and Item='" & Company & "'",connIDMS
	...
	<tr>
		...
		<td align="left" valign="middle">
            ...
            <%  OutFormat="<option value=""#Item#"" #Sel#>#Item#</option>"
                response.write GetConfig("IDMS","order by mark","資訊中心","Item",Unit,"*",OutFormat,UnitA) 
            %>
			</select>
		</td>
		...
		<td align="left" valign="middle">
			...
			<%	dim opA : OutFormat="<option value=""#Item#"" #Sel#>#Item#</option>"
                response.write GetConfig("IDMS","order by mark","電腦操作課","Item",OPin,"*",OutFormat,opA)
			%>  
			</select>
		</td>
	...
	<tr style="display:<% if IO="I" then response.write "none" %>">    
		...
		<td>
	  		<select name="selOPout" size="1" class="option">
				<option value=""></option>
			<%	OutFormat="<option value=""#Item#"" #Sel#>#Item#</option>"
                response.write GetConfig("IDMS","order by mark","電腦操作課","Item",OPout,"*",OutFormat,opA)
			%>  
			</select>  
...
<br><br>
【通行類型】：<br>
A類人員：資訊中心人員或專案、駐點廠商等依授權清冊所列可不需通知、不需陪同之各課授權人員。<br>
