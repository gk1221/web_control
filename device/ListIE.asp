...
if DBaction="d" then connDevice.execute "delete from Device where CreateDate='" & CreateDate & "'"

'if YM="" then YM=DT(now-30,"yyyy/mm")
if YM="" then YM=DT(now,"yyyy/mm")
%>
...
if SQL=empty then
	if SQLsearch="" and YM<>"" then
		YY=mid(YM,3,2) : MM=mid(YM,6,2)
	else
		YY=mid(year(now),3) : MM=month(now) : if MM<10 then MM="0" & MM		
	end if
	 SQL="Select * from device where CreateDate like '" & YY & MM & "%' order by CreateDate Desc"
else
	SQL=replace(SQL,"$","%")	'% is defined by URL,so use $ and then replace $ by %
end if
...
while not rs.eof
	...
	if burn="" then
		textHTML="<tr onmouseover=me.style.color='#9900FF' onmouseout=me.style.color='' style='cursor:pointer' onClick='Select_Click """ & CreateDate & """'>"
	else
		textHTML="<tr>"
	end if	
    textHTML=textHTML &  "<td title='" & CreateDay & " "  & CreateTime & "'><font size='2'>" & CreateDay & "</font></td>" 
...
dim SelRow : SelRow=0
Sub cmdBurn_onClick()
	window.open "ListIE.asp?SQLsearch=" & "<%=request("SQLsearch")%>" & "&SQL=" & "<%=replace(SQL,"%","$")%>" _
...
Sub Order_Click(order)
	...
	if pos>0 and "<%=Sel%>"="none" then
		if order="CreateDate" then
			window.open "ListIE.asp?SQLsearch=y&SQL=" & mid(SQL,1,pos+8) & order & "&Sel=<%=Sel%>","_self"
		else
			window.open "ListIE.asp?SQLsearch=y&SQL=" & mid(SQL,1,pos+8) & order & ",CreateDate&Sel=<%=Sel%>","_self"
		end if
	elseif pos>0 and "<%=burn%>"="" then		
		if order="CreateDate" then
			window.open "ListIE.asp?SQLsearch=y&SQL=" & mid(SQL,1,pos+8) & order & "&YM=" & textShowMonth.value & "&Sel=<%=Sel%>","_self"
		else
			window.open "ListIE.asp?SQLsearch=y&SQL=" & mid(SQL,1,pos+8) & order & ",CreateDate&YM=" & textShowMonth.value & "&Sel=<%=Sel%>","_self"
		end if	
	end if
End Sub
...
Sub Select_Click(CreateDate)
	...
	if "<%=Sel%>"="Y" then
		opener.open "device.asp?CreateDate=" & CreateDate & "&YM=<%=YM%>&DBaction=m","_self"
		window.close
	elseif "<%=Sel%>"="full" then
		...
		if YN=false then
			...
			opener.window.frames(0).open "device/ListIE.asp?Sel=none&SQL=" & SQL,"_self"
		end if			
		window.close
